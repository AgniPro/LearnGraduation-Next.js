...
async function getPopularPost() {
  const res = await fetch(api+"/api/popular-posts");
  if (!res.ok) {
    return [];
  }
  return res.json()
  ...
  return (
          <div className="widget PopularPosts" data-version={2} id="PopularPosts00">
            ...
            <div className="itemPp" role="feed">
              {firstPost ? <>
                <article className="itm mostP" id={firstPost._id}>
                  <div className="iThmb pThmb">
                    <Link className="thmb" href={"/p/" + firstPost.url}>
                      <img alt={firstPost.title} className="imgThm lazy loaded" src={firstPost.image} lazied="true" />
